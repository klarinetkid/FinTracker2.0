...
export class CategorySelectionManager {
    ...
    constructor(state?: [Category[], React.Dispatch<React.SetStateAction<Category[]>>]) {
    ...
    removeCategory = (cat: Category) => {
        if (this.isSelected(cat)) {
            const index = this.selectedCategories.map(c => c.id).indexOf(cat.id)
            const newCategories = [...this.selectedCategories]
...
export const CategorySelectionContext = createContext(new CategorySelectionManager());